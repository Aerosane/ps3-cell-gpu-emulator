...
__global__ void k_rasterTriangles(uint32_t* __restrict__ dst,
    ...
    for (uint32_t t = 0; t < triangleCount; ++t) {
        ...
        // 2*signed-area under the edge-function convention (positive for
        // CCW triangles; all edge funcs > 0 inside when CCW). This is
        // e2 evaluated at v2 where e2 is the edge-v0v1 function.
        ...
        // Inside test — accept both winding orders (area sign flips all three).
...
int CudaRasterizer::init(uint32_t width, uint32_t height) {
    ...
    CU_CHECK(cudaMalloc(&fb_.d_color, bytes));
    CU_CHECK(cudaMemset(fb_.d_color, 0, bytes));
    return 0;
}
...
uint32_t CudaRasterizer::drawTriangles(const RasterVertex* verts,
    ...
    dim3 gs((fb_.width + bs.x - 1) / bs.x,
            (fb_.height + bs.y - 1) / bs.y);
    k_rasterTriangles<<<gs, bs>>>(fb_.d_color, fb_.width, fb_.height,
                                  d_v, tris, blendEnable_ ? 1 : 0);
    cudaDeviceSynchronize();
    cudaFree(d_v);
