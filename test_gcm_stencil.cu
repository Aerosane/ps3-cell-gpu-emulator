// test_gcm_stencil.cu — PPC-driven two-pass stencil masking.
// Pass 1: draw upper-half triangle, write stencil=1 (test ALWAYS, op REPLACE).
// Pass 2: draw full-screen green triangle, test stencil==1 (op KEEP).
// Expected: upper half painted green, lower half untouched (clear color).
...
int main() {
    std::printf("══════════════════════════════════════════════════════\n");
    std::printf("  PPC two-pass stencil masking\n");
    std::printf("══════════════════════════════════════════════════════\n");
    ...
    // Upper-half covering triangle (big triangle whose footprint is the
    // top half of the screen). Y grows downward in RSX pixel space.
    constexpr uint32_t VB_POS_A = 0x100000;
    constexpr uint32_t VB_COL_A = 0x101000;
    float posA[9] = {
        -100.f,     -100.f, 0.5f,
         W+100.f,   -100.f, 0.5f,
         W * 0.5f,  H*0.5f, 0.5f,
    ...
    constexpr uint32_t VB_POS_B = 0x102000;
    constexpr uint32_t VB_COL_B = 0x103000;
    float posB[9] = {
        -100.f,    H+100.f, 0.5f,
    ...
    // --- PASS 1: write stencil=1 in upper half ------------------------
    // Disable depth; enable stencil: func=ALWAYS, ref=1, mask=0xFF,
    // op_zpass=REPLACE so passing fragments stamp 1.
    emit_method(code, NV4097_SET_DEPTH_TEST_ENABLE,    0);
    emit_method(code, NV4097_SET_STENCIL_TEST_ENABLE,  1);
    emit_method(code, NV4097_SET_STENCIL_FUNC,         0x207u);   // ALWAYS
    emit_method(code, NV4097_SET_STENCIL_FUNC_REF,     1u);
    emit_method(code, NV4097_SET_STENCIL_FUNC_MASK,    0xFFu);
    emit_method(code, NV4097_SET_STENCIL_MASK,         0xFFu);
    emit_method(code, NV4097_SET_STENCIL_OP_FAIL,      0x1E00u);  // KEEP
    emit_method(code, NV4097_SET_STENCIL_OP_ZFAIL,     0x1E00u);  // KEEP
    emit_method(code, NV4097_SET_STENCIL_OP_ZPASS,     0x1E01u);  // REPLACE

    emit_method(code, NV4097_SET_VERTEX_DATA_ARRAY_FORMAT + 0 * 4,
                (12u << 8) | (3u << 4) | VERTEX_F);
    emit_method(code, NV4097_SET_VERTEX_DATA_ARRAY_OFFSET + 0 * 4, VB_POS_A);
    emit_method(code, NV4097_SET_VERTEX_DATA_ARRAY_FORMAT + 3 * 4,
                (4u << 8) | (4u << 4) | VERTEX_UB);
    emit_method(code, NV4097_SET_VERTEX_DATA_ARRAY_OFFSET + 3 * 4, VB_COL_A);
    ...
    // --- PASS 2: test stencil==1, paint green -------------------------
    emit_method(code, NV4097_SET_STENCIL_FUNC,         0x202u);   // EQUAL
    emit_method(code, NV4097_SET_STENCIL_FUNC_REF,     1u);
    emit_method(code, NV4097_SET_STENCIL_OP_ZPASS,     0x1E00u);  // KEEP

    emit_method(code, NV4097_SET_VERTEX_DATA_ARRAY_OFFSET + 0 * 4, VB_POS_B);
    emit_method(code, NV4097_SET_VERTEX_DATA_ARRAY_OFFSET + 3 * 4, VB_COL_B);
    emit_method(code, NV4097_SET_BEGIN_END, PRIM_TRIANGLES);
    emit_method(code, NV4097_DRAW_ARRAYS,  0x02000000u);
    emit_method(code, NV4097_SET_BEGIN_END, 0u);
    ...
    CHECK(rs.drawCallCount >= 2, "Two draws dispatched (stencil write + stencil read)");
    CHECK(rs.stencilTestEnable, "Stencil test enabled");

    std::vector<uint32_t> fb(W * H, 0);
    raster.readbackPlane(0, fb.data());

    uint32_t upperGreen = 0, lowerGreen = 0, lowerClear = 0, upperRed = 0;
    for (uint32_t y = 0; y < H; ++y) {
        for (uint32_t x = 0; x < W; ++x) {
            uint32_t p = fb[y * W + x];
            uint8_t R = (p>>16)&0xFF, G = (p>>8)&0xFF, B = p&0xFF;
            bool green = (R < 60  && G > 200 && B < 60);
            bool red   = (R > 200 && G < 60  && B < 60);
            bool clear = ((p & 0x00FFFFFFu) == 0x00202020u);
            bool upper = (y < H / 2);
            if (upper && green)  ++upperGreen;
            if (upper && red)    ++upperRed;
            if (!upper && green) ++lowerGreen;
            if (!upper && clear) ++lowerClear;
        }
    }
    std::printf("  upperGreen=%u upperRed=%u lowerGreen=%u lowerClear=%u\n",
                upperGreen, upperRed, lowerGreen, lowerClear);

    CHECK(upperGreen > 8000,
          "Upper half painted green where stencil==1 AND covered by pass-2 triangle");
    CHECK(upperGreen > upperRed * 5,
          "Green dominates red in upper half (pass-2 overwrote pass-1 where stencil==1)");
    CHECK(lowerGreen == 0,
          "Lower half received no green (stencil test blocked)");
    CHECK(lowerClear > (W * H / 2) * 8 / 10,
          "Lower half preserved clear color");
